...
window.addEventListener('scroll', function () {
  // 스크롤 위치 감지
  var scrollPosition = window.scrollY;
  var targetPosition = 6350; // 원하는 위치
  var maxPosition = 6400; // 최대 위치
  var newsPageBottom = document.getElementById('news_page_bottom');
  var isNewsPageBottomVisible = getComputedStyle(newsPageBottom).opacity === '1';
  var isNewsPageBottomVisibles = getComputedStyle(newsPageBottom).display === 'none';
...
hiddenBtn.addEventListener('mouseover', function () {
  hiddenBtn.src = "./img/페이지꾸미기/on랜턴.png";
});
hiddenBtn.addEventListener('mouseout', function () {
...
for (var i = 0; i < starCount; i++) {
  ...
  star.style.animationDelay = "".concat(Math.random() * 2, "s");
...
var videoTimer;
introVideoElement.addEventListener("loadeddata", function () {
...
introVideo.addEventListener("transitionend", function (event) {
  ...
  if (event.target === introVideo) {
    ...
    if (transform === "matrix(2, 0, 0, 2, 0, 0)") {
      console.log("introVideo가 확대됨을 인식하고 조건 진행");
